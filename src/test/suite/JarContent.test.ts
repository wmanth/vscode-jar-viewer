...
suite("JarContent", () => {
	test("without content", () => {
		const jarContent = new JarContent('/jars/test.jar', []);
	...
	test("with file", () => {
		const jarContent = new JarContent('/jars/test.jar', ['file.txt']);
		expect(jarContent.files.length, 'Archive has 1 file').equals(1);
		expect(jarContent.packages.length, 'Archive has no packages').equals(0);

		const file = jarContent.files.pop();
		checkNameAndUri(file, 'file.txt', 'jar:/jars/test.jar!/file.txt');
	});

	test("with file in subfolder", () => {
		const jarContent = new JarContent('/jars/test.jar', ['foo/bar/file.txt']);
		expect(jarContent.files.length, 'Archive has 1 file').equals(1);
		expect(jarContent.packages.length, 'Archive has no packages').equals(0);
		...
		checkNameAndUri(fooFolder, 'foo', 'jar:/jars/test.jar!/foo');
		...
		checkNameAndUri(barFolder, 'bar', 'jar:/jars/test.jar!/foo/bar');

		const file = barFolder.files.pop() as model.File;
		expect(model.isFile(file), "'file.txt' is a file").true;
		checkNameAndUri(file, 'file.txt', 'jar:/jars/test.jar!/foo/bar/file.txt');
	});

	test("with Java class in package", () => {
		const jarContent = new JarContent('/jars/test.jar', ['net/wmanth/test.class']);
		...
		checkNameAndUri(javaPackage, 'net.wmanth', 'jar:/jars/test.jar!/net/wmanth');

		const javaClass = javaPackage!.classes.pop();
		checkNameAndUri(javaClass, 'test.class', 'jar:/jars/test.jar!/net/wmanth/test.class');
	});
	...
	test("with file in subfolder in package", () => {
		...
		checkNameAndUri(etcFolder, 'etc', 'jar:/jars/test.jar!/net/wmanth/etc');

		const file = etcFolder.files.pop();
		expect(model.isFile(file)).true;
		checkNameAndUri(file, 'config.yaml', 'jar:/jars/test.jar!/net/wmanth/etc/config.yaml');
	});
});
